...
class Player:
    ...
    def move(self, roll, players):
        ...
        if self.in_jail:
            ...
            if self.get_out_of_jail_free:
                ...
                if use_card == 'y':
                    self.get_out_of_jail_free = False
                    self.in_jail = False
                    print(f"{self.token} used 'Get Out of Jail Free' card.")
        ...
        if current_square['name'] == "Chance":
            ...
            print(f"{self.token} moved to square {current_square['square']}: {current_square['name']} (${current_square.get('price', 'N/A')})")
    ...
    def mortgageProperty(self):
     ...
     if self.is_computer:
        # Computer player logic to select a property to mortgage
        property_to_mortgage = self.properties[0]
        ...
        print(f"{self.token} (computer) mortgaged {property_to_mortgage['name']} for ${mortgage_value}")
     else:
        # Human player logic
    ...
    def calculateRent(self, property):
        if property['name'] in ["Go", "Chance", "Community Chest"]:
            return 0  # No rent for these special squares
